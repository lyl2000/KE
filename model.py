...
class Model(object):
    ...
    def __init__(self, nh1, nh2, ny, nz, de, cs, lr, lr_decay, embedding, max_gradient_norm, 
                 model_cell='rnn', model='basic_model', nonstatic=False):
        ...
        inputs = tf.compat.v1.nn.dropout(inputs, rate=self.keep_prob, name='drop_inputs')
        ...
        with tf.compat.v1.variable_scope('output_sy'):
            w_y = tf.compat.v1.get_variable("softmax_w_y", [nh1, ny])
        ...
        with tf.compat.v1.variable_scope('loss'):
            ...
            loss1 = tf.compat.v1.nn.sparse_softmax_cross_entropy_with_logits(logits=sy, labels=label_y)
            label_z = tf.reshape(self.input_z, [-1])
            loss2 = tf.compat.v1.nn.sparse_softmax_cross_entropy_with_logits(logits=sz, labels=label_z)
